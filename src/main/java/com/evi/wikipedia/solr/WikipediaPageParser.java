 ...
 * </ul>
 */
public class WikipediaPageParser implements Closeable{
	private static class NoMoreDataException extends Exception {

		/**
		 * 
		 */
		private static final long serialVersionUID = -11479816108247183L;
	...
	private class Parser extends DefaultHandler implements Runnable {
		...
		String time(String original) {
			...
			buffer.append('-');
			buffer.append(months[Integer.valueOf(original.substring(5, 7))
					.intValue() - 1]);
			buffer.append('-');
				...
				while (!stopped) {
					...
					if (localFileIS != null) { // null means fileIS was closed
						...
						try {
							...
							synchronized (WikipediaPageParser.this) {
								if (localFileIS != is) {
	...
	public WikipediaPageParser(File f) {
		this.file = f;
	}
	...
	public synchronized WikiPage getNextWikiPage()
			throws NoMoreDataException, IOException {
		String[] tuple = parser.next();
		WikiPage docData = new WikiPage.Builder()
			.withId(tuple[ID])
			.withBody(tuple[BODY])
			.withDate(tuple[DATE])
			.withTitle(tuple[TITLE])
			.withURL(toUrl(tuple[TITLE]))
			.build();
		return docData;
	}

	private String toUrl(String title) {
		return String.format("%s/%s","http://en.wikipedia.org",title.replace(" ","_"));
	}
